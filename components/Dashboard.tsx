...
const Dashboard: React.FC = () => {
    ...
    const handleLogToday = async () => {
        ...
        try {
            // Note: The core streak logic should be handled by a Postgres function/trigger in Supabase 
            // for atomicity and consistency. This function would be triggered on insert into 'submissions'.
            // It would check the user's last submission and update the 'current_streak' and 'longest_streak'
            // in the 'profiles' table accordingly.
            ...
            // Re-fetch data to show updated streaks
            await fetchData();
        } catch (err: any) {
            setError(err.message);
        } finally {
    ...
    if (loading) {
        return <div className="flex justify-center items-center h-screen"><div className="w-16 h-16 border-4 border-dashed rounded-full animate-spin border-emerald-500"></div></div>;
    }
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 font-sans">
            <header className="flex justify-between items-center mb-8">
                ...
                <button onClick={signOut} className="text-sm font-medium text-gray-400 hover:text-white bg-gray-800 px-4 py-2 rounded-lg transition-colors">
                    Sign Out
                </button>
            </header>
            ...
            <main>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="md:col-span-1 bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl flex items-center space-x-4 border border-gray-700">
                        <FireIcon />
                        <div>
                    ...
                    <div className="md:col-span-1 bg-emerald-800/30 border border-emerald-600 p-6 rounded-xl flex items-center justify-center">
                        <button 
                            onClick={handleLogToday}
                            disabled={hasLoggedToday || isLogging}
                            className="w-full h-full text-lg font-bold bg-emerald-600 text-white rounded-lg px-6 py-4 transition-transform transform hover:scale-105 disabled:bg-emerald-800 disabled:cursor-not-allowed disabled:transform-none disabled:opacity-70"
                        >
                            {isLogging ? "Logging..." : (hasLoggedToday ? "Completed Today!" : "I Coded Today!")}
                        </button>
                    </div>
                </div>
